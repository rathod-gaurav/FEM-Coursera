...
double forcingFunction(double x){
  return 1e11*x;
}
...
:
  fe (FE_Q<dim>(order), dim), //FE_Q is a dealII object
...
double FEM<dim>::basis_gradient(unsigned int node, double xi){
  ...
  for(unsigned int i = 0; i <= basisFunctionOrder; i++){
      if (std::abs(xi - xi_at_node(i)) < tol) {
          ...
          if(i == node) {
              double sum = 0.0;
              for(unsigned int j = 0; j <= basisFunctionOrder; j++){
                  if(j != node)
...
void FEM<dim>::generate_mesh(unsigned int numberOfElements){
  ...
  // Point in dealII is a location vector
  ...
  for (;elem!=endc; ++elem){
    ...
    for(unsigned int A=0; A<dofs_per_elem; A++){
      ...
      for(unsigned int q=0; q<quadRule; q++){
        ...
        //EDIT - Define Flocal.
        f_integral += basis_function(A,quad_points[q])*quad_weight[q]*forcingFunction(x);
      }
      Flocal[A] = (Area*h_e*0.5)*f_integral;
    }
...
double FEM<dim>::l2norm_of_error(){
  ...
  for (;elem!=endc; ++elem){
    ...
    for(unsigned int q=0; q<quadRule; q++){
      ...
      if (prob==1){
        u_exact = -fbar*pow(x,3)/(6*E) - (1/L)*(g1 - g2 - (fbar*pow(L,3))/(6*E))*x + g1; 
      }
      else if (prob==2){
        u_exact = -fbar*pow(x,3)/(6*E) + ((traction/(E*Area)) + ((fbar*pow(L,2))/(2*E)))*x + g1;
      }
      
			l2norm_integral += pow((u_h - u_exact),2)*quad_weight[q]; 
    }
    l2norm += (h_e/(2*L))*l2norm_integral;
  }
